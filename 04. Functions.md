...
Python provides a **built-in module called `math`** that contains many **mathematical functions** and constants commonly used in calculations.
Before using it, you must import the module:
...
## Complete Table of Python’s `math` Module
This table containing all `math` module constants and functions together in a clean, organized format.

| Name               | Type          | Description                  |
| ------------------ | ------------- | ---------------------------- |
| `e`                | Constant      | Euler’s number (≈ 2.71828)   |
...
| `trunc(x)`         | Number        | Remove decimal part          |
| `fabs(x)`          | Number        | Absolute value (float)       |
| `fmod(x, y)`       | Number        | Floating-point remainder     |
| `modf(x)`          | Number        | Returns (fraction, integer)  |
| `remainder(x, y)`  | Number        | IEEE remainder               |
| `frexp(x)`         | Number        | Returns (mantissa, exponent) |
| `ldexp(x, i)`      | Number        | x × 2^i                      |
| `isfinite(x)`      | Check         | True if finite               |
| `isinf(x)`         | Check         | True if infinity             |
| `isnan(x)`         | Check         | True if NaN                  |
| `gcd(a, b)`        | Number theory | Greatest common divisor      |
| `lcm(a, b)`        | Number theory | Least common multiple        |
| `copysign(x, y)`   | Number theory | Copy sign of y to x          |
| `prod(iterable)`   | Number theory | Multiply all items           |
| `perm(n, k)`       | Number theory | Permutations                 |
| `comb(n, k)`       | Number theory | Combinations                 |
| `nextafter(x, y)`  | Float         | Next float toward y          |
| `ulp(x)`           | Float         | Unit in last place           |
| `exp(x)`           | Exponential   | e^x                          |
| `expm1(x)`         | Exponential   | e^x − 1                      |
| `log(x)`           | Logarithm     | Natural log (base e)         |
| `log(x, base)`     | Logarithm     | Logarithm with base          |
| `log1p(x)`         | Logarithm     | log(1+x) accurately          |
| `log2(x)`          | Logarithm     | Log base 2                   |
| `log10(x)`         | Logarithm     | Log base 10                  |
| `pow(x, y)`        | Power         | x^y (float result)           |
| `sqrt(x)`          | Root          | Square root                  |
| `sin(x)`           | Trigonometry  | Sine                         |
| `cos(x)`           | Trigonometry  | Cosine                       |
| `tan(x)`           | Trigonometry  | Tangent                      |
| `asin(x)`          | Trigonometry  | Arcsine                      |
| `acos(x)`          | Trigonometry  | Arccosine                    |
| `atan(x)`          | Trigonometry  | Arctangent                   |
| `atan2(y, x)`      | Trigonometry  | atan(y/x) with quadrant      |
| `hypot(x, y, ...)` | Trigonometry  | Euclidean distance           |
| `degrees(x)`       | Conversion    | Radians → Degrees            |
| `radians(x)`       | Conversion    | Degrees → Radians            |
| `dist(p, q)`       | Conversion    | Distance between points      |
| `sinh(x)`          | Hyperbolic    | Hyperbolic sine              |
| `cosh(x)`          | Hyperbolic    | Hyperbolic cosine            |
| `tanh(x)`          | Hyperbolic    | Hyperbolic tangent           |
| `asinh(x)`         | Hyperbolic    | Inverse hyperbolic sine      |
| `acosh(x)`         | Hyperbolic    | Inverse hyperbolic cosine    |
| `atanh(x)`         | Hyperbolic    | Inverse hyperbolic tangent   |
| `erf(x)`           | Special       | Error function               |
| `erfc(x)`          | Special       | Complement of erf            |
| `gamma(x)`         | Special       | Gamma function               |
| `lgamma(x)`        | Special       | log(gamma(x))                |
